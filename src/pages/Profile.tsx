              ...
              {activePlans.length === 0 && individualSubjects.length === 0 && premiumPurchases.length === 0 ? (
                ...
                <div className="grid grid-cols-1 gap-4">
                  ...
                  {activePlans.map((plan) => {
                    ...
                    const planDaysLeft = planExpiry ? Math.max(0, Math.ceil((planExpiry.getTime() - Date.now()) / (1000 * 60 * 60 * 24))) : null;
                    const planIsExpired = planDaysLeft !== null && planDaysLeft <= 0;
                    ...
                    return (
                      <motion.div
                        ...
                        initial={{ opacity: 0, y: 10 }}
                        animate={{ opacity: 1, y: 0 }}
                        className={`bg-white rounded-xl border ${planIsExpired ? 'border-destructive/30' : 'border-border'} shadow-sm overflow-hidden group hover:shadow-md transition-shadow`}
                      >
                        <div className="p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                          <div className="flex items-start gap-4 flex-1">
                            <div className={`w-11 h-11 rounded-xl flex items-center justify-center shrink-0 ${planIsExpired ? 'bg-destructive/10 text-destructive' : 'bg-primary/10 text-primary'}`}>
                              <Crown className="w-5 h-5" />
                            </div>
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-2 flex-wrap">
                                <h3 className="text-base font-bold text-foreground">{plan.plan_name}</h3>
                                {planIsExpired ? (
                                  <span className="px-2 py-0.5 rounded-full bg-destructive/10 text-destructive text-[10px] font-bold border border-destructive/20">EXPIRED</span>
                                ) : (
                              ...
                              <div className="flex items-center gap-1.5 mt-1 text-sm text-muted-foreground">
                                ...
                                {planExpiry ? (
                                  planIsExpired ? (
                                    <span className="text-destructive">Expired on {planExpiry.toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}</span>
                                  ) : (
                                    <span className={planDaysLeft !== null && planDaysLeft < 30 ? 'text-amber-600 font-medium' : ''}>{planDaysLeft} days remaining</span>
                                  )
                                ) : (
                          ...
                          <div className="shrink-0">
                            {!planIsExpired ? (
                              <Link
                                to="/#exams-section"
                                className="inline-flex items-center gap-1.5 px-5 py-2.5 bg-primary text-primary-foreground rounded-lg text-sm font-semibold hover:bg-primary/90 transition-all shadow-sm"
                              >
                                Continue
                              ...
                              <Link
                                to="/exams"
                                className="inline-flex items-center gap-1.5 px-5 py-2.5 bg-muted text-muted-foreground rounded-lg text-sm font-semibold hover:bg-muted/80 transition-all"
                              >
                                Renew Plan
                              </Link>
                            )}
