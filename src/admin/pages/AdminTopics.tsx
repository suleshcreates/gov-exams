...
import {
    ArrowLeft, Plus, Edit2, Trash2, Video, GripVertical, Upload, Loader2, FileQuestion
} from "lucide-react";
...
export default function AdminTopics() {
    ...
    const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.files && e.target.files[0]) {
            ...
            // Try to guess duration (optional/advanced, skipping for now or defaults to 0)
        }
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (selectedFile) {
            try {
                setUploading(true);
                videoUrl = await adminService.uploadTopicVideo(selectedFile);
