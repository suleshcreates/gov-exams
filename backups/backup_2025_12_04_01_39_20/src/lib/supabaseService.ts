...
export const supabaseService = {
  ...
  async getActiveStudentPlans(phone: string) {
    logger.debug('[getActiveStudentPlans] Fetching plans for phone:', phone);
    ...
    if (error) {
      logger.error('[getActiveStudentPlans] Error with OR query:', error);
      // Fallback: filter in memory if OR query fails
      const allPlans = await this.getStudentPlans(phone);
      logger.debug('[getActiveStudentPlans] Fallback - all plans:', allPlans);
      const filtered = allPlans.filter(plan => !plan.expires_at || new Date(plan.expires_at) > new Date());
      logger.debug('[getActiveStudentPlans] Fallback - filtered plans:', filtered);
      return filtered;
    }
    logger.debug('[getActiveStudentPlans] Plans retrieved:', data);
    return data || [];
  },
